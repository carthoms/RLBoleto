...
function TRLBanco707.GetImagem : Byte;
begin
   Result := Imagem;
end;

function TRLBanco707.GetNomeBanco : string;
begin
   Result := NomeBanco;
end;

function TRLBanco707.CalcularDigitoNossoNumero(ATitulo: TRLBTitulo) : string;
var
   ACodigoAgencia,
   ANumeroConta,
   ACarteira,
...
function TRLBanco707.GetCampoLivreCodigoBarra(ATitulo: TRLBTitulo) : string;
var
   ACarteira,
   ANossoNumero,
   ADigitoNossoNumero,
...
function TRLBanco707.GerarRemessaCNAB400(var ACobranca: TRLBRemessa; var Remessa: TStringList) : boolean;
var
   ACedenteTipoInscricao, ASacadoTipoInscricao,
   ATipoOcorrencia,
   AEspecieDocumento,
   Registro, NotaFiscal : string;
...
begin
   ...
   begin
      ...
      Remessa.Add('0'+ // IDENTIFICAÇÃO DO REGISTRO HEADER
                  ...
                  '190644331000' + // Codigo Empresa
                  {Formatar('1964',4,false,'0')+ // AGÊNCIA MANTENEDORA DA CONTA
                  '44'+ // COMPLEMENTO DE REGISTRO
                  Formatar('33100',5,false,'0')+ // NÚMERO DA CONTA CORRENTE DA EMPRESA
                  Formatar('0',1)+ // DÍGITO DE AUTO CONFERÊNCIA AG/CONTA EMPRESA}
...
//            toRemessaBaixarPagoCedente: ATipoOcorrencia := '34';
         ...
         if Titulos[NumeroRegistro].Referencia <> '1' then
           Registro := Registro + Formatar(Titulos[NumeroRegistro].NumeroDocumento + '/' + Titulos[NumeroRegistro].Referencia,10)
         else
        ...
        //Registro := Registro + Formatar('',34,true,' '); // NOME DO SACADOR/AVALISTA
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.Nome,34,true,' '); // NOME DO SACADOR/AVALISTA
         Registro := Registro + '      '; // DATA DE MORA
         Registro := Registro + '00'; // PRAZO
         Registro := Registro + '0'; // BRANCOS
         Registro := Registro + Formatar(IntToStr((NumeroRegistro+1)*2),6,false,'0'); // Nº SEQÜENCIAL DO REGISTRO NO ARQUIVO
         ...
         NotaFiscal := '4';
         NotaFiscal := NotaFiscal + Formatar(Titulos[NumeroRegistro].NumeroDocumento,15,false); // Nº DO DOCUMENTO
         NotaFiscal := NotaFiscal + FormatCurr('0000000000000',Titulos[NumeroRegistro].ValorDocumento * 100); // Nº DO DOCUMENTO
         NotaFiscal := NotaFiscal + FormatDateTime('ddmmyyyy',Titulos[NumeroRegistro].DataDocumento); // DATA DE VENCIMENTO DO TÍTULO
         NotaFiscal := NotaFiscal + Formatar(Titulos[NumeroRegistro].ChaveAcesso,44); // Nº DO DOCUMENTO
         NotaFiscal := NotaFiscal + Formatar(' ',313); // COMPLEMENTO DE REGISTRO
         NotaFiscal := NotaFiscal + Formatar(IntToStr((NumeroRegistro*2)+3),6,false,'0'); // Nº SEQÜENCIAL DO REGISTRO NO ARQUIVO

         NumeroRegistro := NumeroRegistro + 1;
         Remessa.Add(NotaFiscal);
      ...
      Remessa.Add('9'+
                  Formatar('',393,true,' ')+
                  Formatar(IntToStr((NumeroRegistro*2)+2),6,false,'0'));
   end;

   Result := TRUE;
end;

function TRLBanco707.GerarRemessa(var ACobranca: TRLBRemessa; var Remessa: TStringList) : boolean;
begin
...
function TRLBanco707.LerRetornoCNAB400(var ACobranca: TRLBRemessa; Retorno: TStringList) : boolean;
var
  ACodigoBanco,
  ANomeCedente,
...
function TRLBanco707.LerRetorno(var ACobranca: TRLBRemessa; Retorno: TStringList) : boolean;
var
  ACodigoBanco,
  ANomeCedente,
  ATipoInscricao : string;
...
function TRLBanco707.VerificaOcorrenciaOriginal(sOcorrenciaOriginal: String): String;
begin
...
function TRLBanco707.VerificaMotivoRejeicaoComando(sMotivoRejeicaoComando: String): String;
var
 i: Integer;
begin
 Result:='';
